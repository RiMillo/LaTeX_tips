...
\end{verbatim}
    ...
    \item Skipping and filtering data:
      \begin{itemize}
        ...
        \item Filtering according to value: One can set up a custom filter; once the filter is run, \texttt{pgfplots} checks if \verb|\pgfmathresult| is empty, if it's the case, the coordinate is discarded
          \begin{verbatim}
% Discard if >4
y filter/.code={
  \ifnum#1>4
