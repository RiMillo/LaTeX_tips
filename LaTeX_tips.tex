...
\end{frame}
\end{verbatim}
      \item \verb|\appendix| may issue some warnings. See \href{https://tex.stackexchange.com/questions/192686/hyperref-warning-caused-by-beamer-appendix}{here}
    \end{itemize}
...
\end{verbatim}
    \begin{itemize}
      \item You are discouraged to use such an option.
      ...
      \item A text will appear in the title to signal that there are more than one slide. You can modify the content as follows: for instance for a text appearing only from the second slide and print the slide number in capital roman number preceded by a dash
\begin{verbatim}
...
\end{verbatim}
        \texttt{beamer} will then print a button of type \texttt{goto} with the text \texttt{To my frame} inside, which, when clicked, will bring us to the 3\textsuperscript{rd} version (slide) of the frame labeled \texttt{myframe}.
    \end{itemize}
  ...
  \item You know what's fun about about \texttt{PowerPoint}? Slide transitions! (Yeah, possibly fun, but not always professional). You have them in beamer too, although, they will appear in full-screen mode only. Look'em up in the user guide, let us give here just an example
\begin{verbatim}
...
\end{verbatim}
  \item For \texttt{verbatim}, algorithms typesetting or even \texttt{tikz} picture (see \autoref{sec:tikzpgf}) the option \texttt{fragile} of the frame should be used.
  \item \texttt{beamer} does nasty things under the hood and we do not want to know. If you are bound to use \verb|#1| or alike, and you get some errors about \verb|\test| or \verb|\iterate| use instead \verb|####1| (yes, you read it right, 4 \verb|#|'s). Have a look at this \href{https://tex.stackexchange.com/a/420454/206129}{answer}.
  ...
  \item To place text exactly where you wish, you lay want to check out package \texttt{textpos}, see \fullref{sec:Sectioning} and don't forget option \texttt{absolute}.
\end{itemize}
...
\end{verbatim}
  \item The key \verb|.| (dot) will repeat the previous overlay number, making the item appear at the same time as the previous one. An example:
\begin{Verbatim}[samepage=true]
\begin{itemize}
...
\end{verbatim}
  ...
  \item Overlays and \texttt{pgfplots}: see \autoref{sec:tikzpgf} (and its child, \autoref{sub:pgfplots}).
  \item Tables
    \begin{itemize}
...
\end{verbatim}
        \end{itemize}
      \item Access the value of a key wit \verb|\pgfkeyvalueof{<key>}|
    \end{itemize}
  \item Environments:
    \begin{itemize}
    ...
    \end{itemize}
  \item Align inline \texttt{tikz} with normal text:
\begin{verbatim}
Lorem \tikz[baseline=(C.base)]\node[draw=red] (C) {ipsum}; dolor
\end{verbatim}
  \item Computations: it is possible to do (more or less) complex computations in \texttt{pgf}. We give the bases below, but you are invited to see the manual
    \begin{itemize}
      \item Load library \texttt{fpu} for floating point computations (like, double precision), otherwise the basic routines are limited to $\pm16384$.
      \item Commands:
        \begin{itemize}
          \item \verb|\pgfmathparse{<expr>}|: evaluate the expression passed as argument and store it in \verb|\pgfmathresult|. The result is a decimal.
          \item \verb|\pgfmathsetmacro{\<macro>}{<expr>}|: evaluate the expression passed as argument and store it in \verb|\<macro>|. The result is a decimal. It could be used to both define or redefine a macro.
          \item \verb|\pgfmathtruncatemacro{\<macro>}{<expr>}|: As above but the result is an integer
          \item \ldots
        \end{itemize}
      \item Warning: \verb|\pgfmathresult| is temporary and it is sometimes used by \texttt{pgf} itself under the hood, for instance when computing point coordinates. Hence, unless your are going to use the result just after the evaluation, you'd better use \verb|\pgfmathsetmacro| or alike
      \item The expression accepts:
        \begin{itemize}
          \item Standard mathematical functions: \verb|+-*/^|, \verb|sin(.)|, \verb|sqrt(.)|, \verb|>=<|, \verb|log(.)|, \verb|e|, \verb|round(.)|\ldots
          \item Several number formats: \verb|0.00005|, \verb|5e-5|
          \item Units \verb|2pt+1cm|. However, the results will be given without units, unless commands \verb|\pgfmathsetlength[macro]| are used
          \item Brackets: \verb|[i]| to access the \texttt{i}-th element of an array (array are defined with braces \verb|{.}|)
          \item (Pseudo) random functions: \verb|rnd(.)|,\ldots
          \item \ldots
        \end{itemize}
      \item Functions: they can be used in \texttt{pgfplots}, see below. Some examples in \href{run:./ex_plot.tex}{\texttt{ex\_plot.tex}}
        \begin{itemize}
          \item Command \verb|\pgfmathdeclarefunction{<name>}{<algorithm code>}|
          \item Key: \verb|declare function={<function declarations>}|
        \end{itemize}
    \end{itemize}
  \item Size: one CANNOT choose the exact size of the final \texttt{tikz} picture: differently from, for instance, including a picture, options like \verb|width=0.5\linewidth| do not exists. In alternative, one can scale the picture
\begin{verbatim}
\begin{tikzpicture}[scale=0.5]
  % My beatiful TikZ picture here
\end{tikzpicture}
\end{verbatim}
    \begin{itemize}
      \item \texttt{scale} has a major drawback: it acts on drawings but not on text: try option \verb|every node/.style={scale=<n>}|, more info \href{https://tex.stackexchange.com/questions/26846/how-to-scale-a-tikzpicture-including-texts}{here}.
    \end{itemize}
  \item Drawing lines:
    \begin{itemize}
      ...
      \item Relative coordinates: this enables one to build coordinate relative to previously defined ones. Two commands: \verb|++| relative and update the point in the path, \verb|+| relative and \textbf{do not} update the point in the path. Hence, the following commands give the same result
\begin{verbatim}
\draw (A) -- ++(1cm,1cm) % relative to A
        -- ++(1cm,1cm);% relative to second pt
\draw (A) --  +(1cm,1cm) % relative to A
        --  +(2cm,2cm);% relative to A
\end{verbatim}
      \item Here is how to bend a line (give it a curved look). Adjust the direction (\texttt{left} or \texttt{right}) and the angle to suit your wishes.
\begin{verbatim}
\draw (0,0) to[bend left=45] (10,0);
\end{verbatim}
      \item Corners: Two examples (taken from \texttt{TikZ} manual)
...
\end{verbatim}
  \item Nodes\ldots
\begin{verbatim}
\node[<opt>] at (<point>) {<Description>}; or
\draw[<opt>] node[<opt>] {<Description>};
\end{verbatim}
    \begin{itemize}
      \item Colors: \texttt{draw=<clr>} sets the color of the edge of the node, \texttt{text=<clr>} sets the color of the description, \texttt{fill=<clr>} sets the color of the node background, simply \texttt{<clr>} is like fill.
      \item Opacity: in order to different level of opacity for the background and the text, use, respectively, \texttt{opacity} and \texttt{text opacity}
      \item Font: \verb|font={<font>}|
      \item Width and text alignment: by default, \texttt{tikz} writes everything on the same line. One can force a width, to make it go to newline: \texttt{text width=5cm}. Then, one might select the alignment: \verb!align=left|center|right!
      \item \texttt{rounded corners}
    \end{itemize}
...
\end{pgfonlayer}
\end{Verbatim}
  \item Filling area between two curves (for the fun of it, we also use patterns):
    \begin{itemize}
      \item Use \texttt{fill} with \texttt{even odd rule}, see \href{https://tex.stackexchange.com/a/412600/206129}{here}

        \begin{minipage}[T]{.68\textwidth}
          \begin{verbatim}
\draw[red,thick] (-1.2,-1.2) -- (1.2,1.2);
\fill[gray,even odd rule,pattern=crosshatch] %
            (0,0) circle (1.2) %
            (0,0) circle (1.0);
          \end{verbatim}
        \end{minipage} \hfill%
        \begin{minipage}[T]{.28\textwidth}
          \begin{tikzpicture}
            \draw[red,thick] (-1.2,-1.2) -- (1.2,1.2);
            \fill[gray,even odd rule,pattern=crosshatch] (0,0) circle (1.2)
                                      (0,0) circle (1);
          \end{tikzpicture}
        \end{minipage}

      \item Use the \texttt{pgfplots} utility \verb|\[tikz]fillbetween|, see \href{https://tex.stackexchange.com/questions/411568/warning-tikzfillbetween-and-change-pattern}{here} (see also the trick about the layer to avoid warnings) or \href{https://tex.stackexchange.com/questions/381591/using-tikzfillbetween-to-color-between-two-curves}{here}

        \begin{minipage}[T]{.68\textwidth}
          \begin{verbatim}
\usetikzlibrary{intersections,pgfplots.fillbetween}
\draw[red,thick] (-1.2,-1.2) -- (1.2,1.2);
\path[name path=A] (0,0) circle (1.2);
\path[name path=B] (0,0) circle (1.0);
...
\end{Verbatim}
  ...
  \item Externalization: for big pictures it is advised to use the \texttt{TikZ} library \texttt{external} in order to avoid regenerating the picture ad each compilation. It basically create separates pdf that will be included in the followings compilations.
    \begin{itemize}
      ...
      \item If strange errors occurs:
        \begin{itemize}
          \item Check that you passed the \verb|-shell-escape| option. If you had it but the errors persist, see below.
          \item Check that you are not using language that modify some characters (for instance, French and "\texttt';", see \fullref{sec:languages} and the \texttt{babel} item at the beginning of this section),
          \item Try disabling the optimization: pass option \texttt{optimization=false} when activating the externalization (see below).
        \end{itemize}
...
\begin{itemize}
  ...
  \item Plot combinations of usual mathematical functions or use \texttt{pgf} function defined as shown above. In the following example, we control the number of points and the domain of the function (see also \href{run:./ex_plot.tex}{\texttt{ex\_plot.tex}})
\begin{verbatim}
\addplot[samples=20,domain=-pi:pi] {1+sin(2*x)};
\end{verbatim}
  ...
  \item Sizes\ldots
    \begin{itemize}
      \item Change the size: \texttt{width=<len>} (similar for \texttt{height}).
      \item Set default size: \verb|\axisdefaultwidth{<len>}| (similar for \texttt{height}).
      \item \verb!scale only axis=true|false!: default is \texttt{false}. Is false, \texttt{pgfplots} will try to make the whole plot, that is labels and ticks included, fit the given width / height. If true, only the axis is scaled.
    \end{itemize}
  \item Axis style:
    \begin{itemize}
      \item \verb!axis x line=box|top|middle|center|bottom|none!. A star \verb!axis x line*! means no arrow. Similarly for $y$- and $z$-axis, e.g., \verb!axis y|z line! but with \texttt{left} and \texttt{right} (instead of \texttt{top} and \texttt{bottom}). For every line \verb|axis lines|.
      \item \verb|every outer x axis line/.append style={...}|: Set stile of axis, e.g., color, arrow heads...
      \item \verb|axis line shift=3pt|: Shift the axis away from the plot. Without unit, i.e., \verb|...=3|, it means in terms of plot units
      \item \verb|separate axis lines=true|: Enables separate path commands for every axis line, valid only if box mode.
    \end{itemize}
  \item Lines\ldots
    \begin{itemize}
  ...
  \item Markers\ldots
    \begin{itemize}
      \item \texttt{only marks} vs. \texttt{no markers} / \texttt{mark=none}.
      ...
      \item Use markers outside \texttt{TikZ} environments: \verb|\pgfuseplotmark{<marker_code>}|, for instance \verb|\pgfuseplotmark{square*}| (a \verb|\raisebox{<how_much>}{<what>}| may be needed). However, use \verb|\label{<label>}| and \verb|\ref{<ref>}| when possible, see the legend section below.
    \end{itemize}
  ...
  \item Legend\ldots
    \begin{itemize}
      ...
      \item Referencing a plot: one can give a \verb|\label{<lab>}| to a plot and recover with the usual tools \verb|\ref{<lab>}|. This enables one to manually build a legend. For instance consider put a bunch of labels then writing a table outside the \texttt{tikz} scope explaining them. Or even inline references: ``\verb|the results (\ref{res} in the plot)...|''
\begin{verbatim}
\addplot[red] {sin(x)};\label{pgf:sin}
\end{verbatim}
      \item Exclude plot from legend (and cycles): option \verb|forget plot|.
      \item Only lines / marks: sometimes, styles \texttt{draw=none} and \texttt{mark=none} are not well interpreted by the legend (it happens that lines are drawn in the legend even if one used \texttt{draw=none}). Instead, try \texttt{no markers} and \texttt{only marks}.
...
\end{verbatim}
      ...
      \item Interesting options: \texttt{reverse legend} (invert the order)
    \end{itemize}
  \item \verb!unbounded coords=jump|discard!: choose how to deal with bad data (NaNs). Suppose you try to plot the following data: (0,1,NaN,3,4). With \texttt{discard} you end up with a continuous plot, from 1 to 3 directly. Whereas with \texttt{jump}, the plot will not be continuous: there will be a first line joining 0 and 1, and a second one joining 3 and 4.
...
\begin{groupplot}[%
  group style={%
    group size=2 by 1, % n_rows by n_cols
  },
...
\end{verbatim}
    \begin{itemize}
      ...
      \item Command \verb|\nextgroupplot| indicates switching the following plot
        ...
        \end{itemize}
      \item Use \texttt{x|ymod=log} for logarithmic plots
    \end{itemize}
  \item Different $y$-axis: see related example in \href{run:./ex_plot.tex}{\texttt{ex\_plot.tex}}. The main idea is to make \texttt{pgfplots} draw two plots/axes one on top of the other. Some caution is of order:
    \begin{itemize}
      \item Use \texttt{scale only axis} in axis option to force equal dimensions for both axes.
      \item Force the two plots to have the same $x$ limits.
      \item Tell \texttt{pgfplots} to draw in the same place by calling \verb|\pgfplotsset{set layers}| just before the first axis.
    \end{itemize}
    ...
    \begin{itemize}
      \item They are computed automatically but you can force them:
        \begin{itemize}
          ...
          \item \texttt{xticklabels={1e0,Marco,2}}: choose what is printed. The version \texttt{ytick} is available as well. Set the style with \texttt{xticklabel style}.
            \begin{itemize}
              \item MIND: \texttt{xticklabel={1e0}} (singular) will set the same label for all the ticks
            \end{itemize}
...
\end{verbatim}
  ...
  \item Help grids:
    \begin{itemize}
      ...
      \item \verb!x|ycomb!: draw a line from a plot point to the \verb!x|y! axis. An alternative could be using error bars: \verb|[error bars/]x fixed relative=1,x dir=minus| for both x and y
    \end{itemize}
  ...
  \item Fill area between two lines: \href{http://pgfplots.net/tikz/examples/fill-between-plots/}{here} is a really cool example.
    \begin{itemize}
      \item Use \verb|soft clip={domain=<lower>:<upper>}| to limits to a certain domain
      \item To fill an entire vertical strip of the plot use \texttt{tikz} \texttt{rectangle} action and access the \texttt{y} limits. E.g.\ fill between \texttt{x=-2} and \texttt{x=2}
\begin{verbatim}
\fill[red] (-2,\pgfkeysvalueof{/pgfplots/ymin})
  rectangle (+2,\pgfkeysvalueof{/pgfplots/ymax});
\end{verbatim}
    \end{itemize}
  \item Layers: the order in which the plots are written the \texttt{.tex} file is also the order in which the plots are drawn; which means that the first plot is drawn over by all following plots. One can tweak layers using something similar to what has been discussed in the previous section. One may also want to check out this \href{https://tex.stackexchange.com/questions/350458/pgfplots-understanding-how-to-draw-plots-on-different-layers-and-arrange-them}{SO question}; main ideas: define layers in a \texttt{pgfplotsset}, then use them in combination with option \texttt{on layer=}.
  ...
  \end{itemize}
  \item \texttt{tikz} and \texttt{beamer}
    \begin{itemize}
      \item If you get weird errors, try the \texttt{fragile} option \verb|\begin[fragile]{Title}|.
      \item \texttt{beamer} does nasty things under the hood and we do not want to know. If you are bound to use \verb|#1| or alike, and you get some errors about \verb|\test| or \verb|\iterate| use instead \verb|####1| (yes, you read it right, 4 \verb|#|'s). Have a look at this \href{https://tex.stackexchange.com/a/420454/206129}{answer}.
      \item Overlays and \texttt{pgfplots} can give great satisfaction, however it might not always work, especially if using fancy overlays like \verb|\uncover|. The one that will always work (well, let's say "should always work", shall we?) is \verb|\only|. This \href{https://tex.stackexchange.com/a/269558/206129}{answer} offers a workaround for \verb|\alt|, and it could be used with \verb|\temporal|: in fact, that is the core code you should use for these fancy things.
      \item Some convenient definitions are given in the library \verb|overlay-beamer-styles|, part of the \href{https://mirrors.chevalier.io/CTAN/graphics/pgf/contrib/aobs-tikz/aobs-tikz.pdf}{\texttt{aobs-tikz}} package. An \href{https://tex.stackexchange.com/questions/175228/how-can-i-make-beamer-overlays-with-tikz-node-attributes}{example}.
      \item For legends, use options as follows: \verb|\addentrylegend[visible on<>]{}|
    \end{itemize}
\end{itemize}
...
Two are the main reasons to use (and to love) \texttt{pgfplotstable}: creating plots from data using \verb|\addplot table {<table>}|, typesetting tables with \verb|\pgfplotstabletypeset{<table>}| (mind that this is a just an interface and \texttt{pgfplotstable} will only print the content in a format compatible with a \texttt{tabular} or alike environment). Examples are in \href{run:./ex_plot.tex}{\texttt{ex\_plot.tex}}.
  ...
  \begin{itemize}
    ...
    \item When using \texttt{pgfplotstable} with plots, one can use:
      \begin{itemize}
        \item Labels, if the table has headers
\begin{verbatim}
\addplot table[x=<head_x>,y=<head_y>] {<table>};
\end{verbatim}
        \item Indexes (zero-based)
\begin{verbatim}
\addplot table[x index=<n_x>,y index=<n_y>] {<table>};
\end{verbatim}
        \item Expressions (use \verb|\thisrow{.}|, \verb|\thisrowno{.}| to refer to the data, see below; mathematical functions accepted by \texttt{pgf} are accepted, see above). Example: plot column \texttt{ysq} squared against the values of the first (index 0) column multiplied by 3
\begin{verbatim}
\addplot table[x expr={3*\thisrowno{0}},y expr={\thisrow{ysq}^2}] {<table>};
\end{verbatim}
      \end{itemize}
    \item \verb|\pgfplotstabletypeset{<table>}| just translates the table into something readable by \TeX{}. By default is a \texttt{tabular}. You can change the environment with \texttt{begin table} and \texttt{end table}. An example
\begin{verbatim}
\pgfplotstableset{
...
\end{verbatim}
      \begin{itemize}
        \item You can see the \TeX{} code that \texttt{pgfplotstable} produces by passing the option \texttt{outfile=<name>.tex}: \texttt{pgf} creates a file with the same name and containing the actual \TeX{} code. This option can be useful if debugging or if you want to re-use the table elsewhere.
      \end{itemize}
    \item \texttt{pgfplotstable} works column-wise. Need to work by row? Use (see \href{https://tex.stackexchange.com/questions/183519/plotting-data-from-rows-instead-of-columns-in-latex}{here})
\begin{verbatim}
...
\end{verbatim}
    ...
    \item Sort: options \verb|sort,sort key={<key>}| and also \href{https://tex.stackexchange.com/questions/66177/how-can-i-filter-select-data-from-a-table-and-plot-it}{here}.
    \item Skipping and filtering data:
      \begin{itemize}
        \item Consider only any other $n$ data points: \verb|each nth point=<n>|
        \item \verb|skip coords between index={<n>}{<m>}|
        \item Filtering according to value: One can set up a custom filter; once the filter is run, \texttt{pgfplots} checks if \verb|\pgfmathresult| is set, if it's the case, the coordinate is discarded
          \begin{verbatim}
% Discard if >4
y filter/.code={
  \ifnum#1>4
  ...
  \fi
          ...
          \end{verbatim}
        \item Filter according to another column value: see \href{https://tex.stackexchange.com/questions/98003/filter-rows-from-a-table}{here}. Mind that this can be easily used in \texttt{addplot}, however for typesetting the table, it's more complicated and refer to the link.
          \begin{verbatim}
% Discard if the value of a row is different from a reference one
filter on col/.style 2 args={
  x filter/.code={
    \edef\colCurVal{\thisrow{#1}}
    \edef\colRefVal{#2}
    \ifx\colCurVal\colRefVal
    \else
      \def\pgfmathresult{}
    \fi
  }
        ...
        \item Restrict domain:
          \begin{verbatim}
% Trhow all samples outside [1, 15] away
restrict y to domain=1e0:1.5e1,
% Similarly, but with the row, unprocessed data
restrict expr to domain={rawy}{1e0:1.5e1},
    ...
    \item Get cell content: For current row (replace \verb|this| with \verb|prev| for previous row or with \verb|next| for next row)
      \begin{itemize}
        \item {}[Not exactly the same] \verb|\coordindex| (0-based): yields the current index of the table row (starting with 0). This does not count header or comment lines. For instance, imagine you have a file with only a column (see next section \autoref{ssub:pgfplotstable} for \texttt{table} usage) to be used as \texttt{y} and you want to plot it against 1,2,\ldots
          \begin{verbatim}
\addplot table[x expr=\coorindex+1,y index=0] {table.dat};
          \end{verbatim}
...
\end{verbatim}
      ...
      \item Another way for having place-holder pictures is via the package \texttt{todonotes} (see below) and its command
\begin{verbatim}
...
\end{verbatim}
  ...
  \item Fancy and colorful todo notes from the \href{http://tug.ctan.org/macros/latex/contrib/todonotes/todonotes.pdf}{\texttt{todonotes}} package.
    \begin{itemize}
      \item Checkout this
        \href{https://tex.stackexchange.com/a/313337/206129}{answer} for an in-house todo-list. A checked-box is available also with \texttt{fontawesome} (see \fullref{sec:Miscellaneous}).
    \end{itemize}
  \item With \texttt{beamer}: \autoref{sec:Beamer}.
  \item Watermark: add text in the background
    \begin{itemize}
      \item Package \href{https://distrib-coffee.ipsl.jussieu.fr/pub/mirrors/ctan/macros/latex/contrib/draftwatermark/draftwatermark.pdf}{\texttt{draftwatermark}}: basic and simple, no fuss. Some options (there are also commands. Defaults are given)
        \begin{itemize}
          \item \texttt{angle=45}
          \item \texttt{text=Draft}
          \item \texttt{vpos}/\texttt{hpos}: positions (default is center)
          \item \texttt{color}
        \end{itemize}
      \item Package \href{https://ctan.tetaneutral.net/macros/latex/contrib/xwatermark/doc/xwatermark-guide.pdf}{xwatermark}: does everything.
    \end{itemize}
...
\end{verbatim}
  ...
  \item Package \href{https://mirror.ibcp.fr/pub/CTAN/macros/latex/contrib/eso-pic/eso-pic.pdf}{eso-pic} may be used to generate help grids in order to better manage the placing and the layout. A typical call would be
\begin{verbatim}
\usepackage[texcoord,grid,gridunit=cm,
  gridcolor=black!30,subgridcolor=gray!10]
  {eso-pic}
\end{verbatim}
\end{itemize}
...
The \texttt{hyperref} package enables to create hyperlinks. Here is an \href{https://www.overleaf.com/learn/latex/hyperlinks}{overview} (the previous word is indeed a hyperlink! Click on it!), you might also want to check \href{https://en.wikibooks.org/wiki/LaTeX/Hyperlinks}{this} out.
\begin{itemize}
  \item The basic command is \verb|\href{<link_address>}{<place_holder>}|: \verb|place_holder| is what is printed in the document and \verb|link_address| is the actual address.
    \begin{itemize}
      ...
      \item Email address: prepend \texttt{mailto:}, e.g.\ \verb|\href{mailto:jbl_pdg@gmail.com}{JBL}|.
      \item Local file: prepend \texttt{run:}, e.g.\ \verb|\href{run:/path/to/tips.pdf}{tips}|.
    \end{itemize}
  \item \verb|\url{http://my-web.site}|: type website full name and add a link
  \item \verb|\hyperref[label_name]{text}|: type \texttt{text} and add a link to the \LaTeX{} label \verb|label_name|. For instance, \verb|\hyperref[sec:following]{in the following section}|
  \item One can customize the links color (options \verb!url|linkbordercolor!), disable the color (\texttt{hidelinks}), use an underlined version instead of a box (\verb|pdfborderstyle={/S/U/W 1}|), color the text of the link (no box is printed, \texttt{colorlinks=ture} and \verb!url|linkcolor!). And many others.
...
\begin{itemize}
  \item ATTENTION: \texttt{cleveref} does not like French-\texttt{babel}, especially its symbols such as ``\texttt{:}'' (colon). Hence, one cannot use labels like \verb|eq:euler|, because of the colon. Hence, either one does not use colons, or they should be prepended by \verb|\string|, see the comments of this \href{https://tex.stackexchange.com/a/83818}{post}.
...
\begin{itemize}
  ...
  \item Dedicated \verb|.bib| files are usually edited. These files are usually then translated into lower-level instruction to \LaTeX{}, which are usually stored in a \verb|.bbl|. The structure of the bibliographic items inside a \verb|.bib| file can seem quite strict. Find all the info \href{https://en.wikibooks.org/wiki/LaTeX/Bibliography_Management}{here}. Generally, those items looks like
\begin{verbatim}
@reftype{key,
...
\end{verbatim}
    It is composed of:
    \begin{itemize}
      \item They always start with a ``\verb|@|''; all the lines which do not start with an \emph{at} and which are not part of an item will be ignored
      \item \verb|reftype|: the reference type, \texttt{article}, \texttt{book},\ldots
      \item \verb|key|: the citation key which should be used in cite-commands: \verb|\cite{key}|
      \item A series of fields, in the examples above \texttt{author}, \texttt{title}, and \texttt{year}. The order does not matter. Braces are compulsory (I think). According to the type, some fields are mandatory. According to the style, some fields might not be printed
      \item The special item \verb|@preamble{'\some\latex\code'}| allows one to write \LaTeX{} code, define macro inside the bibliography file
      \item You can usually use \LaTeX{} code inside a field, for instance \verb|author={Cl\'ement}|
      \item Use braces inside a field to keep exactly some characters: \verb|title={A ca{P}ital p inside}| will produce exactly what it says. According to the bibliography style, capitals at the beginning of a word might be changed into lowercase. Use the braces in order to keep them: \verb|title={The {S}tokes equations}|
    \end{itemize}
...
\end{verbatim}
  ...
  \item Set the style: use option
    ...
    \end{itemize}
  \item Sort the citations:
    \begin{itemize}
      \item In the bibliography: \verb|\usepackage[sorting=<style>]{biblatex}|, usually the argument is a combination of \texttt{n} (for name), \texttt{t} (title), \texttt{y} (year) according to the importance (e.g.\ \texttt{nyt}, \texttt{nty}).
      \item When citing: option \verb!sortcites=true|false!
    \end{itemize}
  \item Use \texttt{natbib} commands: option \verb!natbib=true|false!
  ...
  \item In some predefined styles, the mention "\emph{In:}" precedes the article title, for instance. One may remove it by calling: \verb|\renewbibmacro{in:}{}|, as explained \href{https://tex.stackexchange.com/a/10686/206129}{here}. Have a look \href{https://tex.stackexchange.com/questions/10682/suppress-in-biblatex}{here} for a custom solution involving only the articles.
...
\end{verbatim}
  ...
  \item Two-letter initials, for instance Ph.~for Philippe: use \verb|Abba, {\relax Ph}ilippe|
  \item When \texttt{hyperref} (see \autoref{sec:Referencing}) is used, links to the bibliography page are produced. They can be weird, for instance in author-year format only the year is concerned by the link. Have a look \href{https://tex.stackexchange.com/questions/15951/hyperlink-name-with-biblatex-authoryear-biblatex-1-4b}{here} on how to modify this behavior.
  \item Option \verb!uniquelist=true|false|minyear!. I need an example here. Suppose you have two references: \begin{inparaenum}[i)] \item Alice, Bob, Clint, and Don, 12; \item Alice, Bob, Ed, and Florent, 13. \end{inparaenum} Moreover suppose that you have an author-year--like citation styles with \emph{et al.}~enabled after the first author name. Both references would result in: Alice \emph{et al.}, yy. Someone may want to distinguish them, since they have not the same authors. If \verb|uniquelist| is disabled (\verb|false|), they both appear as: Alice \emph{et al.}, yy. If enabled (\verb|true|), \begin{inparaenum}[i)] \item Alice, Bob, Clint, \emph{et al.}, 12; \item Alice, Bob, Ed, \emph{et al.}, 13 \end{inparaenum}; notice as the max-citation name has been overruled in order to show at least one non-unique author. If \verb|minyear|, this happens only if the year is the same (hence, not in our example).
...
\end{verbatim}
    ...
    \end{itemize}
  \item According to the style, the author in the bibliography might be replaced by a dash if it is repeated. One may disable this feature (and always print the author) with the option \texttt{dashed=false}.
...
Have a look at the \href{http://ctan.mines-albi.fr/macros/latex/contrib/biblatex/doc/biblatex.pdf}{manual} for more details about these options/commands and many useful other ones.
...
\end{verbatim}
  ...
  \item Glossary:
    \begin{itemize}
      ...
      \item About \texttt{glossaries}
        \begin{itemize}
          \item How to:
            \begin{enumerate}
              \item Load the package
              \item Use \verb|\makeglossary| in the preamble
              \item Create the entries, e.g.,
\begin{verbatim}
% Acronym: label, shown, description
\newacronym{un}{UN}{United Nations}

% Generic entry: label, description
\newglossaryentry{life}{
  name={Life},
  description={What happens before the death}
  % [other options...]
}
\end{verbatim}
              \item Inside the document, print all the glossaries \verb|\printglossaries|
            \end{enumerate}
          \item It should be loaded \textbf{after} \texttt{hyperref};
          \item Entries in separate file: load \verb|\loadglsentries[<type>]{gloss.tex}|, see \href{https://tex.stackexchange.com/questions/287072/creating-glossary-entries-in-separate-file}{here}
          \item Items are printed only if actually referenced in the document. To add them anyway, use \verb|\glsadd{label}| or \verb|\glsaddall|, see \href{https://tex.stackexchange.com/questions/14903/glossary-with-entries-without-reference}{here}
          \item Use option \texttt{toc} to have an entry in the TOC
          \item If more than one glossaries (e.g., acronyms and standard glossary):
            \begin{itemize}
              \item Print only one type, e.g., \verb|\printglossary[type=\acronymtype]|. No brackets means standard one
              \item Use option \verb|section=section| to avoid clearing double pages between any two glossary type lists
            \end{itemize}
          \item Style
            \begin{itemize}
              \item Use option \verb|style=<astyle>| to set the style. See available styles \href{https://www.dickimaw-books.com/gallery/glossaries-styles/}{here}
              \item Use \texttt{nonumberlist} when loading the package or when printing the glossary to avoid printing the list of the pages where the entry is referenced
            \end{itemize}
          \item Referencing:
            \begin{itemize}
              \item Acronym: \verb|\acrshort{label}|, \verb|\acrlong{label}|, \verb|\acrfull{label}|
              \item Entry: \verb|\gls{label}|, \verb|\Gls{label}| (first letter uppercase), \verb|\glspl{label}| (plural), \verb|\Glspl{label}| (plural, first letter uppercase)
            \end{itemize}
          \item Change title and TOC title: use options \verb|title=<title>, toctitle=<toctitle>| of \verb|\printglossaries|
          \item Group and set group titles: see \href{https://tex.stackexchange.com/questions/99142/customising-glossary-group-titles}{here}
        \end{itemize}
    \end{itemize}
...
\end{Verbatim}
    \begin{itemize}
      \item What if I need to use a two-argument loop in \texttt{pgfplots}? and a three-argument loop? Well, then \verb|\pgfplotsinvokeforeach| is useless and one has to stick to \verb|\foreach| and toy around with the dark side of \TeX, in particular, \verb|\edef| and \verb|\noexpand| are your friends. Hints: define a temporary command with the wished instruction and, most importantly, where no expansion is requested, then use the command:
\begin{verbatim}
\begin{axis}
  \foreach \l\fl\cl in {2/cyan/black,3/red/black,4/yellow/black}{
    \foreach \j in {0,1} {
      \pgfmathsetmacro\crcl{(20-4*\l-2*\j)/2}
      \edef\temp{%
        \noexpand\draw[fill=\fl,color=\cl] (0,0) circle (\crcl);
      }
      \temp
    }
  }
...
\end{verbatim}
    \end{itemize}
  \item An example of looping with more than one arguments and conditionals is in \href{run:./ex_plot.tex}{\texttt{ex\_plot.tex}}.
  \item Storing previous values: use option \texttt{remember as}, \texttt{(initially foo)} sets the initial value
\begin{verbatim}
\foreach \x[remember=\x as \prevx (initially 0)] in {1,2,3}{
  $\prevx < \x$,
}
\end{verbatim}
  \item Get the \emph{index}: \texttt{count}. For instance, to draw a node A at position 1, B at position 2, etc
\begin{verbatim}
\tikz
...
\begin{itemize}
  \item Get the manual of an installed package by running \verb|texdoc <name\_of\_the\_package>|
  \item Define macros/new commands: we treat here the \LaTeX{} mode (more primitive \TeX{} commands such as \verb|\def|, \verb|\let| are available as well but won't be addressed here). Intro \href{https://en.wikibooks.org/wiki/LaTeX/Macros}{here}
    \begin{itemize}
      \item \verb|\newcommand\macro{<definition>}|: defines a new command \verb|\macro| that will expand to \verb|<definition>|. Gives an error if \verb|\macro| is already defined
      \item \verb|\renewcommand\macro{<definition>}|: redefines (overwrite) command \verb|\macro|. Gives an error if \verb|\macro| is not defined yet
      \item \verb|\providecommand\macro{<definition>}|: defines a new command \verb|\macro|. Does nothing if \verb|\macro| is already defined
      \item IDEA: use \verb|\providecommand| for an interactively set parameters from outside the sources as explained \href{https://en.wikibooks.org/wiki/LaTeX/Macros#Command-line_LaTeX}{here}
\begin{verbatim}
pdflatex '\providecommand\draft{draft}\input{source.tex}'
\end{verbatim}
      \item Macros with arguments:
        \begin{itemize}
          \item The arguments number has to be told in brackets before the definition. Arguments are referred to with \verb|#n|, where \verb|n| is the position
\begin{verbatim}
\newcommand\greet[2]{#1, #2!}
\greet{Hello}{World} % Renders "Hello, World!"
\end{verbatim}
          \item Default arguments: one can specify a default argument, in brackets, after the number of total arguments. If a value should be provided, it should be passed with brackets (not braces). Only the first argument can be defaulted
\begin{verbatim}
\newcommand\greet[2][Hello]{#1, #2!}
\greet{World} % Renders "Hello, World!"
\greet[Hi]{World} % Renders "Hi, World!"
\end{verbatim}
          \item For more flexibility with default arguments, additional packages should be used. Have a look at \href{https://www.ctan.org/pkg/xparse}{\texttt{xparse}} as suggested \href{https://tex.stackexchange.com/a/29975/206129}{here}, or at \href{https://www.ctan.org/pkg/xargs}{\texttt{xargs}} as suggested \href{https://texfaq.org/FAQ-twooptarg}{here}
      \end{itemize}
    \end{itemize}
...
\end{verbatim}
  \item Counters: very informative \href{https://fr.overleaf.com/learn/latex/Counters}{page}
    \begin{itemize}
      ...
      \item Modify: several possibilities \verb|\addtocounter{<name>}{<n>}|, \verb|\stepcounter{<name>}|, \verb|\setcounter{<name>}{<n>}|
...
\end{verbatim}
  ...
  \item An exhaustive (?) list of all symbols is available \href{http://tug.ctan.org/info/symbols/comprehensive/symbols-a4.pdf}{here} or via a simple command line \verb|texdoc comprehensive| (beware, hundreds of pages).
  \item For typesetting units the reference package is \href{http://mirror.ibcp.fr/pub/CTAN/macros/latex/contrib/siunitx/siunitx.pdf}{\texttt{siunitx}}
    \begin{itemize}
      \item Predefined commands for units and prefixes: \verb|\milli\meter\per\second\squared| gives \si{\milli\meter\per\second\squared}
        \begin{itemize}
          \item Format for \verb|\per| can be customized as well as the (invisible) product (for instance, add a dot)
          \item Some non-SI units are defined as well: \verb|\degreeCelsius|, \verb|\percent|,\ldots
        \end{itemize}
      \item Basic commands: \verb|\si{<unit>}| (just units) and \verb|\SI{<num>}{<unit>}| (values and units). For angles use \verb|\ang{<num>}|
      \item Lists (highly customizable): \verb|\SIlist{<number1;number2;...>}{<unit>}|
      \item Ranges (highly customizable): \verb|\SIrange{<number1>}{<number2>}{<unit>}|
    \end{itemize}
  ...
  \item Pimping the pdf with interactive stuff
    \begin{itemize}
      \item We have seen the clickable coordinates in \texttt{pgfplots}, \autoref{sub:pgfplots}.
      \item Pop-up's and mouseover effects: package \href{https://mirrors.chevalier.io/CTAN/macros/latex/contrib/fancytooltips/fancytooltips.pdf}{\texttt{fancytooltips}} (supported by \texttt{Adobe} only).
      \item Add annotations and notes to the pdf: package \href{https://ctan.crest.fr/tex-archive/macros/latex/contrib/pdfcomment/doc/pdfcomment.pdf}{\texttt{pdfcomment}}.
    \end{itemize}
  ...
  \item (Not exactly \TeX{}-related) pdf utilities (extract, concatenate, rotate, modify,\ldots): checkout \href{https://github.com/DavidFirth/pdfjam#using}{\texttt{pdfjam}} (easy stuff like concatenate, rotate, extract) and \texttt{pdftk} (adding background/watermark with option \texttt{background} or \texttt{stamp}). The ultimate tool is \href{https://www.ghostscript.com/doc/current/Use.htm}{\texttt{ghostscript}}. Some examples
    \begin{itemize}
      \item Extract and concatenate: Examples of accepted ranges: \texttt{'1,6-9'}, \texttt{'-2,4,6'}
\begin{verbatim}
pdfjam [options] -- file_1.pdf ['<page_range>'] \
                   [file_2.pdf ['<page_range>'] ...]
\end{verbatim}
      \item If you need to extract and fit to the pdf size, use \texttt{ghostscript} (for the pages, the option \texttt{-sPageList} is also available and accepts ranges)
\begin{verbatim}
...
\end{verbatim}
  ...
  \item (Not exactly \TeX{}-related) Collaborative and cloud tools for \TeX{}:
    ...
    \end{itemize}
  \item \LaTeX-\texttt{vim} set up: see \href{https://www.ejmastnak.com/tutorials/vim-latex/intro/}{this well detailed article}
    \begin{itemize}
      \item \href{https://github.com/lervag/vimtex}{\texttt{vimtex}} plugin
      \item Backward-search
        \begin{itemize}
          \item \href{https://github.com/gauteh/vim-evince-synctex}{\texttt{vim-evince-synctex}} plugin, but it might not always work
          \item For setting up \texttt{okular}, see this \href{https://docs.kde.org/stable5/en/okular/okular/inverse_search.html}{page} and use the suggested configuration of \texttt{vimtex} (in \texttt{vim}, run \verb|:he VimtexInverseSearch|):
\begin{verbatim}
vim -v --not-a-term -T dumb -c "VimtexInverseSearch %l '%f'"
\end{verbatim}
          \item \href{https://pwmt.org/projects/zathura/}{zathura} pdf-reader is often recommended on Unix
        \end{itemize}
    \end{itemize}
%
\end{itemize}
