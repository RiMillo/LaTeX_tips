...
\begin{itemize}
  ...
  \item Space after paragraph: \verb|\setlegth{\parskip}{<len>}|
...
\end{verbatim}
  ...
  \item Table and colors: the reference is \href{http://mirrors.ircam.fr/pub/CTAN/macros/latex/contrib/colortbl/colortbl.pdf}{\texttt{colortbl}}. Some tips \href{https://texblog.org/2011/04/19/highlight-table-rowscolumns-with-color}{here};
  \item For typesetting numerical data (but nocessarily), you may want to have a look at \texttt{pgfplotstable}, \autoref{sec:tikzpgf}.
\end{itemize}
...
\end{verbatim}
    \end{itemize}
  \item It has been seen \autoref{Items} that \texttt{beamer} and \texttt{enumitem} may interfere, typically with this latter erasing the settings done by the former. Here how to restore the \texttt{beamer} bullets:
\begin{Verbatim}[samepage=true]
...
\end{Verbatim}
  ...
  \item Include other slides in pdf: see the bullet about \texttt{pdfpages} in \ref{Miscellaneous}.
  ...
  \item \texttt{includeonly}-like (\autoref{Sectioning}): define labels and filter as shown \href{https://tex.stackexchange.com/questions/4370/disabling-included-pictures-in-beamer}{here}
\begin{verbatim}
...
\end{verbatim}
        ...
        \item Legend outside the plot: add \texttt{legend pos=outer <cardinal position>} (example: \texttt{outer north east}), or specify the position: \verb|legend style={at={(1,1)},anchor=north\}|
        ...
        \item Add lines not related to plot (for example for a legend title): first, use \verb|\addlegendimage{empty legend}|, the use \verb|\addlegendentry{<lorem>}| to fill the newly line. Of course the order is important
...
Two are the main reasons to use/love \texttt{pgfplotstable}: creating plots from data using \verb|\addplot table {<table>};|, typesetting tables with \verb|\pgfplotstabletypeset{<table>}| (mind that this is a just an interface and \texttt{pgfplotstable} will only print the content in a format compatible with a \texttt{tabular} or alike environment). Examples are in \href{run:./ex_plot.tex}{\texttt{ex\_plot.tex}}.
  ...
  \begin{itemize}
    ...
    \item Create columns: two main ways
      \begin{itemize}
        \item Online: dedicated option, the column is typeset directly 
\begin{verbatim}
...
\end{verbatim}
  ...
  \item With \texttt{beamer}: \autoref{Beamer}.
...
\begin{itemize}
  ...
  \item Some tips already given in \autoref{sec:math} and \autoref{Items}
...
\begin{itemize}
  ...
  \item option \texttt{noabbrev} makes \texttt{cleveref} print full reference namea, e.g. \texttt{equation} instead of \texttt{eq.}
...
\begin{itemize}
  \item Management (\texttt{include} and alike): see \autoref{Sectioning}, and \href{https://en.wikibooks.org/wiki/LaTeX/Modular_Documents}{here}.
  \item The \texttt{book}-class special structure: see \autoref{Sectioning}.
  \item Table of Content in each chapter: package \href{http://texdoc.net/texmf-dist/doc/latex/minitoc/minitoc.pdf}{\texttt{minitoc}}. Remark: it is built for the \texttt{book} class, it may work with \texttt{memoir} thanks to a \href{https://tex.stackexchange.com/questions/291743/minitoc-of-wrong-chapter}{workaround}.
    ...
    \end{itemize}
  \item Bibliography in each chapter: see \autoref{Bibliography}. We suggest using \texttt{biblatex}, especially if you want per-chapter and global references at the same time, have a look \href{https://tex.stackexchange.com/questions/168713/global-and-per-chapter-bibliographies}{here} or \href{https://tex.stackexchange.com/questions/168713/global-and-per-chapter-bibliographies}{here}.
...
\begin{itemize}
  ...
  \item One very basic option is to use the \texttt{verbatim} utility of \TeX~(see \autoref{Miscellaneous} for some tips). It is the right choice if one has to put a couple of lines and if one does not bother with syntax-highlighting.
\end{itemize}
...
\end{verbatim}
  ...
  \item Include external pdf: \href{http://ctan.tetaneutral.net/macros/latex/contrib/pdfpages/pdfpages.pdf}{\texttt{pdfpages}} and \verb|\includepdf{my.pdf}|, have a look \href{https://stackoverflow.com/questions/2739159/inserting-a-pdf-file-in-latex}{here}.
    \begin{itemize}
      \item With beamer \ref{Beamer}, things are a bit trickier: reset the background canvas then include your pdf
\begin{verbatim}
